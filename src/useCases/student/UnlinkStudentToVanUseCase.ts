...
> {
  ...
  public static create(
    ...
    return new UnlinkStudentToVanUseCase(studentRepository, vanRepository);
  }
  ...
  public async execute(
    ...
    const van = await this.vanRepository.findById(student.vanId!);
    if (!van) {
      return left(new VanNotFoundError(student.vanId!));
    }
    if (van.driverId !== input.requesterId) {
      return left(new NotVanOwnerError(van.id));
    }
